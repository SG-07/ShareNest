...
export default function ItemDetails() {
  ...
  useEffect(() => {
    ...
    async function fetchItem() {
      try {
        ...
        console.error(err);
        setError(err?.response?.data?.message || err.message || 'Unable to load item');
      } finally {
  ...
  const handleRequest = async () => {
    try {
      setRequesting(true);
      // example payload (you can expand as per backend API)
      ...
      setError(err?.response?.data?.message || err.message || 'Unable to create request');
    } finally {
      setRequesting(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4">
      <div className="bg-white rounded shadow p-6">
        <div className="md:flex gap-6">
          <img src={item.image || '/placeholder-item.png'} alt={item.name} className="w-full md:w-1/2 h-80 object-cover rounded" />
          <div className="mt-4 md:mt-0 md:flex-1">
            ...
            <div className="mt-6 flex items-center gap-4">
              <span className={`px-3 py-1 rounded text-sm ${item.available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {item.available ? 'Available' : 'Unavailable'}
              </span>
              <button
                onClick={handleRequest}
                disabled={!item.available || requesting}
                className="ml-auto bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 disabled:opacity-60"
              >
